...
router.post('/product', function (req, res, next) {
    ...
    connection.query('insert into product values(?, ?, ?, ?, ?, ?)',
        [body.PL_id, body.PL_name, body.PL_price * 1.2, body.PL_price, body.number, body.S_id],
        function (err, rows) {
            if (err) {
                console.log(err.message);
                connection.query('UPDATE product SET P_quantity = P_quantity + ' + body.number
                + ' WHERE P_id = ' + body.PL_id,
                function(err, rows){
                    if(err){
                        res.end(err.message);
                    }
